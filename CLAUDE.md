...
- **SIEMPRE bump de versión en `package.json`** después de cada cambio que se mergea a main. No esperar a que el usuario lo pida. Usar semver: patch para fixes/refactors, minor para features, major para breaking changes.
- **NUNCA pushear directo a `main`** sin bypass de admin. Usar PRs con squash & merge.
- Al mergear PRs con `gh pr merge`, usar `--admin` flag para bypass del ruleset de protección.
- Borrar branches después de mergear (`--delete-branch`).
- Respuestas en español, comentarios en código en inglés.
